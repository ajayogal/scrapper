...
class IGAScraper {
    ...
    async searchProducts(query, maxResults = 50) {
        try {
            ...
            // First go to homepage to handle location popup
            await this.page.goto(this.baseUrl, { waitUntil: 'networkidle2' });
            
            // Handle location popup
            try {
                const closeButton = await this.page.$('button[aria-label="close popup"], .close-button, button[class*="close"]');
                if (closeButton) {
            ...
            // Navigate to search
            const searchUrl = `${this.baseUrl}/search/1?q=${encodeURIComponent(query)}`;
            ...
            while (currentResults < maxResults) {
                ...
                if (newHeight === previousHeight && currentResults < maxResults) {
                    ...
                    const nextButton = await this.page.$('.pagination-next, [aria-label="Next page"], button[class*="next"]');
                    if (nextButton) {
            ...
            console.error('Error scraping IGA:', error);
            return [];
        }
    }
    ...
    async getProductDetails(productUrl) {
        try {
            ...
            console.error('Error getting product details from IGA:', error);
            return {};
        }
    }
